...
from models import DatabaseConnection, TableInfo, DatabaseSchema, QueryResponse, QueryType, SavedQuery, ScheduledQuery
...
class DatabaseManager:
    ...
    def create_scheduled_queries_table(self) -> bool:
        """Create the scheduled_queries table if it doesn't exist."""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                ...
                cursor.execute("""
                    SELECT COUNT(*) 
                    FROM INFORMATION_SCHEMA.TABLES 
                    WHERE TABLE_NAME = 'scheduled_queries'
                """)
                ...
                if cursor.fetchone()[0] == 0:
                    ...
                    """)
                    conn.commit()
                    logger.info("Created scheduled_queries table")
                    return True
                else:
                    logger.info("scheduled_queries table already exists")
                    return True
                    
        except Exception as e:
            logger.error(f"Error creating scheduled_queries table: {e}")
            return False
    
    def save_scheduled_query(self, scheduled_query: ScheduledQuery) -> Optional[int]:
        """Save a scheduled query to the database."""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                
                # Ensure table exists
                self.create_scheduled_queries_table()
                
                # Insert query
                cursor.execute("""
                    INSERT INTO scheduled_queries (question, tables_used, schedule_type, schedule_time, schedule_day, cron_expression, is_active)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (
                    scheduled_query.question,
                    ','.join(scheduled_query.tables_used) if scheduled_query.tables_used else '',
                    scheduled_query.schedule_type,
                    scheduled_query.schedule_time,
                    scheduled_query.schedule_day,
                    scheduled_query.cron_expression,
                    scheduled_query.is_active
                ))
                ...
                cursor.execute("SELECT @@IDENTITY")
                query_id = cursor.fetchone()[0]
                logger.info(f"Saved scheduled query with ID: {query_id}")
                return int(query_id) if query_id else None
                
        except Exception as e:
            logger.error(f"Error saving scheduled query: {e}")
            return None
    
    def get_all_scheduled_queries(self, active_only: bool = False) -> List[ScheduledQuery]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                query = """
                    SELECT id, question, tables_used, schedule_type, schedule_time, schedule_day, 
                           cron_expression, is_active, created_at, last_run_at, last_run_status, run_count
                ...
                queries = []
                for row in cursor.fetchall():
                    tables_used = row[2].split(',') if row[2] else []
                    
                    queries.append(ScheduledQuery(
                        id=row[0],
                        question=row[1],
                        tables_used=tables_used,
                        schedule_type=row[3],
                ...
                logger.info(f"Retrieved {len(queries)} scheduled queries")
                return queries
                
        except Exception as e:
            logger.error(f"Error retrieving scheduled queries: {e}")
            return []
    
    def get_scheduled_query_by_id(self, query_id: int) -> Optional[ScheduledQuery]:
        ...
        try:
            ...
            logger.error(f"Error retrieving scheduled query by ID: {e}")
            return None
    
    def update_scheduled_query_run(self, query_id: int, status: str):
        """Update the last run information for a scheduled query."""
        try:
            ...
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    SET last_run_at = ?, last_run_status = ?, run_count = run_count + 1
                    WHERE id = ?
                """, (datetime.now(), status, query_id))
                
                conn.commit()
                logger.info(f"Updated scheduled query {query_id} run status: {status}")
                return True
                
        except Exception as e:
            logger.error(f"Error updating scheduled query run: {e}")
            return False
    
    def update_scheduled_query(self, query_id: int, scheduled_query: ScheduledQuery) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    UPDATE scheduled_queries 
                    SET question = ?, tables_used = ?, schedule_type = ?, 
                        schedule_time = ?, schedule_day = ?, cron_expression = ?, is_active = ?
                    WHERE id = ?
                """, (
                    scheduled_query.question,
                    ','.join(scheduled_query.tables_used) if scheduled_query.tables_used else '',
                ...
                conn.commit()
                logger.info(f"Updated scheduled query {query_id}")
                return True
    ...
    def delete_scheduled_query(self, query_id: int) -> bool:
        """Delete a scheduled query by ID."""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                ...
                if rows_affected > 0:
                    logger.info(f"Deleted scheduled query with ID: {query_id}")
                    return True
                else:
                    logger.warning(f"Scheduled query with ID {query_id} not found")
                    return False
    ...
    def toggle_scheduled_query(self, query_id: int, is_active: bool) -> bool:
        ...
        try:
            with self.get_connection() as conn:
                ...
                conn.commit()
                logger.info(f"Toggled scheduled query {query_id} to {is_active}")
                return True
